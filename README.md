...
└──────────────────────────────────────────────────────────────────────────────┘
                                      ...
                                      ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│                      结构引擎层 (Structure Engines)                            │
│  ┌──────────────────────┐  ┌──────────────────────────┐                      │
│  │ ShortTermStateEngine │  │ AttackOpportunityEngine  │                      │
│  │ 短线状态引擎           │  │ 今日进攻引擎              │                      │
│  │ 主升/退潮/弱转强/恐慌  │  │ TOP5进攻目标池 + 多因子评分 │                      │
│  └──────────────────────┘  └──────────────────────────┘                      │
└──────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
...
- **胶囊式导航**：5 个核心页面通过顶部胶囊按钮切换，每个页面有独立的 Terminal View 编号和信号标签
- **超大市场状态卡片**（`terminal-hero`）：首页展示情绪周期（56px 超大字）、主线、风险等级、一句话 AI 剧本
- **呼吸灯动画**：市场周期旁带脉冲动画，增强终端感
- **风险颜色系统**：主升=绿、分歧=黄、退潮=红、修复=青
- **页面过渡动画**：驾驶舱进场动画（cockpit-enter）
...
### 3. 结构引擎层（Structure Engines）

在因子引擎之上，新增两个规则驱动的结构引擎，杜绝 AI 胡编：

| 引擎 | 功能 |
|------|------|
| **ShortTermStateEngine** | 基于最近 5 日走势，输出统一短线状态（主升加速/突破启动/弱转强/恐慌出货/缩量退潮/健康上行/横盘蓄势），附带双向剧本和得分组件 |
| **AttackOpportunityEngine** | 综合趋势强度(25%)、量能爆发(20%)、龙头/突破信号(20%)、短线状态(15%)、市场情绪适配(20%)，扫描全市场输出 TOP5 进攻目标池 |
...
- **6大危险信号检测**：高位股炸板、中位股A杀、板块成交过热、炸板率飙升、核按钮重现、高位放量滞涨
- **明日观测点生成**：固定3个关键观测指标，含判定标准和双向预案
- **AI每日市场剧本**：结构化JSON格式的市场预判，含情绪周期、主线演化、资金状态、危险信号、观测点
...
赋予系统"时间记忆能力"，从单日分析升级为趋势叙事，包含四个独立引擎：

| 引擎 | 功能 |
|------|------|
| **MarketStateEngine** | 核心大脑，分析市场周期、情绪趋势、板块轮动、龙头切换、风险变化 |
| **MarketSnapshotGenerator** | 每日15:30收盘后自动生成完整市场快照（情绪、涨跌停、龙头、板块、北向资金） |
| **NarrativeEngine** | 将历史数据转换为 AI 可读的结构化摘要，生成市场故事，识别关键转折点 |
| **HistoricalSnapshotFetcher** | 优先本地数据库获取真实历史数据，不足时从 akshare 拉取，支持增量/全量 |
...
│   │   ├── api.py                  # 统一 API 接口
│   │   ├── snapshot_generator.py   # MarketSnapshotGenerator 快照生成器
│   │   ├── insight_engine.py       # NarrativeEngine 市场叙事引擎
│   │   └── historical_fetcher.py   # HistoricalSnapshotFetcher 历史拉取器
│   │
...
│       └── analysis_result.py
│
├── engines/                        # 结构引擎层（规则驱动，杜绝 AI 胡编）
│   ├── __init__.py
│   ├── attack_opportunity_engine.py # AttackOpportunityEngine 今日进攻引擎
│   └── structure_engine/
│       ├── __init__.py
│       └── short_term_state.py     # ShortTermStateEngine 短线状态引擎
│
├── factors/                        # 因子引擎
...
│
├── models/                         # 通用数据模型
│   └── analysis_result.py          # AnalysisResult 分析结果数据类
│
├── pages/                          # 独立页面
│   └── morning_analysis.py         # AI早盘市场分析（独立Streamlit页面）
│
├── ui/                             # UI组件
│   ├── theme.py                    # 深色科技风 CSS 主题（Bloomberg/TradingView 风格）
...
│
├── database/                       # 数据库基础设施
│   ├── db.py                       # SQLite + SQLAlchemy 轻量用户管理
│   └── user_db.sqlite              # 用户认证数据库
│
├── validation/                     # 验证系统（开发中）
│   ├── backtester.py               # 回测框架
...
├── data/cn/daily/                  # CSV 格式本地日线（短线引擎使用）
...
采用 **TradingView/Bloomberg 风格的胶囊式导航**，每个页面有独立的 Terminal View 编号与信号标签。

### Page 01: ◉ 市场状态（Market State）
- **超大市场状态卡片**（`terminal-hero`）：56px 大字显示情绪周期（主升期/分歧期/退潮期…）
- **核心指标一览**：主线板块 + 次主线、风险等级（低/中/高）、涨停/跌停/连板/炸板率
- **一句话 AI 市场剧本**：基于 AI 结构化 JSON 生成的市场预判
- **危险信号内联展示**：6大危险信号自动检测并标注严重程度
- **TOP3 热点板块**：带涨跌幅进度条 + 成交额 + 领涨股
- **自动数据采集**：进入页面自动调用市场情绪引擎，缓存 AI 解析结果

### Page 02: ◎ 个股状态（Stock State）
- **短线状态卡片**（`ShortTermStateEngine`）：基于最近5日走势，输出规则化短线状态（主升加速/弱转强/恐慌出货…），附带双向剧本和得分组件
...
- **AttackOpportunityEngine**：综合趋势强度(25%)、量能爆发(20%)、龙头/突破信号(20%)、短线状态(15%)、市场情绪适配(20%) 多因子扫描
- **TOP5 进攻目标池**：每只目标含综合评分、当前状态、所属主题、风险等级、进攻理由、进攻剧本
- **全市场扫描**：从本地 CSV 日线数据批量计算因子，按评分排序

### Page 04: ◌ 市场时间线（Market Timeline）
- **市场情绪日志**：按日期展开历史情绪快照，含涨停/跌停、涨跌比例、情绪周期
- **个股看盘记录**：按时间倒序展示历史分析，按股票筛选、展开查看详细 AI 复盘
- **下载支持**：每条记录支持 TXT 下载

### Page 05: ▣ 数据中心（Data Center）
- **本地数据湖**：Parquet 格式存储的 A 股日线数据 + CSV 日线副本
...
| 市场状态卡片 | 56px 情绪周期 + 主线 + 风险 + 一句话 AI 剧本 | ✅ |
| 个股短线状态 | ShortTermStateEngine 规则化短线状态引擎 | ✅ |
| 今日进攻 TOP5 | AttackOpportunityEngine 多因子扫描全市场 | ✅ |
| 游客模式 | 默认进入，5次免费分析 | ✅ |
...
| 市场记忆系统 | 4引擎：State+Snapshot+Narrative+Fetcher | ✅ |
| 行情数据 | Parquet数据湖，全量/增量同步 | ✅ |
| 早盘分析 | 独立页面，大盘指数+板块+AI报告 | ✅ |
