"""
admin_auth 管理员后台 - Admin Dashboard
提供管理员专属功能
"""

import streamlit as st

from admin_auth.db import init_db
...
def show_admin_dashboard():
    ...
    if selected_menu == "仪表盘":
        ...
        _show_analysis_records()
    elif selected_menu == "操作日志":
        _show_operation_logs()
...
def _show_dashboard():
    ...
    with col3:
        st.metric("📈 今日分析", stats.today_analysis)

    with col4:
        st.metric("📊 总分析次数", stats.total_analysis)

    st.subheader("👥 最近注册用户")
    users = AdminService.get_all_users()[:10]
        ...
        with st.expander(f"👤 {user.username} ({user.name or '-'})"):
            ...
            with col3:
                ...
                if st.button("更新会员等级", key=f"update_{user.id}"):
                    if AdminService.update_user_membership(user.username, new_membership):
                ...
                if user.is_active:
                    if st.button(f"🔒 封禁用户", key=f"ban_{user.id}"):
                        success, new_status = AdminService.toggle_user_active(user.username)
                        if success:
...
def _show_analysis_records():
    """显示分析记录"""
    st.header("📊 分析记录")

    records = AdminService.get_all_analysis_records()
...
def _show_operation_logs():
    """显示操作日志"""
    st.header("📝 操作日志")

    logs = AdminService.get_all_logs()
